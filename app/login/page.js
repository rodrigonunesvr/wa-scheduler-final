...
export default function LoginPage() {
    ...
    const handleLogin = async (e) => {
        ...
        if (error) {
            setError(error.message === 'Invalid login credentials' ? 'Credenciais invÃƒÂ¡lidas' : error.message)
            setLoading(false)
        } else {
            router.push('/')
        }
                    ...
                    <form className="space-y-6" onSubmit={handleLogin}>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢"
                                    className="appearance-none block w-full pl-10 pr-3 py-3 border border-slate-200 rounded-xl shadow-sm placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-100 focus:border-violet-400 sm:text-sm font-medium transition-all" />
                            </div>
                        </div>
