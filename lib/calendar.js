    ...
    if (existing) {
        for (const apt of existing) {
            ...
            if (checkOverlap(start, end, aptStart, aptEnd)) {
                ...
                return {
                    error: true,
                    message: `Esse horÃƒÂ¡rio jÃƒÂ¡ estÃƒÂ¡ ocupado (${timeStr} - ${apt.customer_name}). Por favor, escolha outro horÃƒÂ¡rio.`
                };
            }
        }
    }
...
export async function updateAppointment({
    ...
    if (fetchError || !apt) {
        return { error: true, message: 'Agendamento nÃƒÂ£o encontrado.' };
    }
