...
export const SERVICE_DURATIONS = {
    'Fibra ou Molde F1': 120,
    'Banho de Gel': 90,
    'Manutenção': 90,
    'Manutenção (outra profissional)': 110,
    'Remoção': 45,
    'Esmaltação Básica': 30,
    'Esmaltação Premium': 45,
    'Esmaltação ou Pó + Francesinha': 45,
    'Esmaltação + Francesinha + Pó': 60
};
...
export async function calculateTotalDuration(services) {
    ...
    const { data: dbServices } = await supabase.from('services').select('name, duration');

    return services.reduce((total, s) => {
        if (dbServices) {
            // Find exact or partial match in DB
            const match = dbServices.find(dbSvc => s.toLowerCase().includes(dbSvc.name.toLowerCase()) || dbSvc.name.toLowerCase().includes(s.toLowerCase()));
            if (match && match.duration) return total + match.duration;
        }

        // Try exact match in fallback
        if (SERVICE_DURATIONS[s]) return total + SERVICE_DURATIONS[s];
...
export async function findAvailableSlots({
    ...
    let slotsFound = 0;
    const maxSlots = 20; // Increased to cover a full day of 30min slots
    ...
    for (let i = 0; i < daysToCheck; i++) {
        ...
        while (timeCursor.hour() < closeHour) {
            ...
            // Always add 30 mins to allow consecutive/overlapping start time options
            timeCursor.add(30, 'minutes');
        }
    }
