...
export async function calculateTotalDuration(services) {
    ...
    const serviceMap = {};
    if (dbServices) {
        dbServices.forEach(s => serviceMap[normalizeString(s.name)] = s.duration_minutes || s.duration || 60);
    }
...
export async function calculateTotalPrice(services) {
    ...
    const serviceMap = {};
    if (dbServices) {
        dbServices.forEach(s => serviceMap[normalizeString(s.name)] = Number(s.price) || 0);
    }
