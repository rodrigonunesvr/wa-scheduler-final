...
export async function calculateTotalDuration(services) {
    ...
    const serviceMap = {};
    if (dbServices) {
        dbServices.forEach(s => serviceMap[normalizeString(s.name)] = s.duration);
    }
    ...
    return services.reduce((total, s) => {
        ...
        for (const [name, dur] of Object.entries(serviceMap)) {
            if (normS.includes(name)) return total + dur;
        }
...
export async function calculateTotalPrice(services) {
    ...
    return services.reduce((total, s) => {
        ...
        for (const [name, price] of Object.entries(serviceMap)) {
            if (normS.includes(name)) return total + price;
        }
    ...
    for (let i = 0; i < daysToCheck; i++) {
        ...
        const [{ data: appointments }, { data: blocks }] = await Promise.all([
            supabase
                ...
                .gte('starts_at', currentDay.clone().startOf('day').toISOString())
                .lte('starts_at', currentDay.clone().endOf('day').toISOString()),
            supabase
                .from('blocks')
                .select('*')
                .gte('starts_at', currentDay.clone().startOf('day').toISOString())
        ...
        while (timeCursor.isBefore(closeTime)) {
            ...
            if (appointments) {
                for (const apt of appointments) {
                    if (checkOverlap(slotStart, slotEnd, moment(apt.starts_at), moment(apt.ends_at))) {
                        conflict = true;
                        break;
                    }
                }
            ...
            if (!conflict && blocks) {
                for (const block of blocks) {
                    if (checkOverlap(slotStart, slotEnd, moment(block.starts_at), moment(block.ends_at))) {
                        conflict = true;
                        break;
                    }
                }
...
export async function bookAppointment({
    ...
    if (existing) {
        for (const apt of existing) {
            if (checkOverlap(start, end, moment(apt.starts_at), moment(apt.ends_at))) {
