    ...
    if (existing) {
        for (const apt of existing) {
            ...
            if (checkOverlap(start, end, aptStart, aptEnd)) {
                ...
                return {
                    error: true,
                    message: `Esse horÃƒÆ’Ã‚Â¡rio jÃƒÆ’Ã‚Â¡ estÃƒÆ’Ã‚Â¡ ocupado (${timeStr} - ${apt.customer_name}). Por favor, escolha outro horÃƒÆ’Ã‚Â¡rio.`
                };
            }
        }
    }
...
export async function updateAppointment({
    ...
    if (fetchError || !apt) {
        return { error: true, message: 'Agendamento nÃƒÆ’Ã‚Â£o encontrado.' };
    }
