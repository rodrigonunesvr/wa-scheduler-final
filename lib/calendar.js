    ...
    if (existing) {
        for (const apt of existing) {
            ...
            if (checkOverlap(start, end, aptStart, aptEnd)) {
                ...
                return {
                    error: true,
                    message: `Esse horÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡rio jÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ estÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ ocupado (${timeStr} - ${apt.customer_name}). Por favor, escolha outro horÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡rio.`
                };
            }
        }
    }

    // Fetch default professional
    const { data: profs } = await supabase.from('professionals').select('id').eq('active', true).limit(1)
    const defaultProfId = (profs && profs.length > 0) ? profs[0].id : null
...
export async function updateAppointment({
    ...
    if (fetchError || !apt) {
        return { error: true, message: 'Agendamento nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o encontrado.' };
    }
