...
export async function findAvailableSlots({
    ...
    for (let i = 0; i < daysToCheck; i++) {
        ...
        while (timeCursor.isBefore(closeTime)) {
            ...
            if (!conflict && blocks && blocks.length > 0) {
                for (const block of blocks) {
                    ...
                    if (checkOverlap(slotStart, slotEnd, bStart, bEnd)) {
                        console.log(`🛑 CONFLITO de Bloco: Slot ${slotStart.format('HH:mm')} sobreposto a BLOQUEIO (${bStart.format('HH:mm')}-${bEnd.format('HH:mm')})`);
                        conflict = true;
                        break;
                    }
                }
            ...
            // Se não houver bloqueio manual, garantimos que entre 12h e 13h não haja agendamento
            const lunchStart = slotStart.clone().hour(12).minute(0);
            const lunchEnd = slotStart.clone().hour(13).minute(0);
            ...
            if (!conflict) {
                availableSlots.push({
                    start: slotStart.toISOString(), // v83: Always UTC ISO
