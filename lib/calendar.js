...
export async function bookAppointment({
    ...
    for (const s of serviceList) {
        ...
        if (!found) {
            return { error: true, message: `O serviço "${s}" não está disponível no momento para agendamento.` };
        }
    }
...
export async function updateAppointment({
    ...
    for (const s of serviceList) {
        const found = activeNames.some(name => s.toLowerCase().includes(name));
        if (!found) {
            return { error: true, message: `O serviço "${s}" não está disponível no momento.` };
        }
    }
