...
export function getOpenHours(dateStr, scheduleRules) {
    ...
    return { openHour, openMin, closeHour, closeMin, hasRule: !!rule };
}
...
export function isTimeValid(start, end, scheduleRules) {
    ...
    const isBeforeOpen = start.isBefore(openTime);
    const isAfterClose = end.isAfter(closeTime);

    if (isBeforeOpen || isAfterClose) {
        console.warn(`[isTimeValid] ❌ Falha: Início=${start.format('HH:mm')} Fim=${end.format('HH:mm')} | Limites=${openTime.format('HH:mm')}-${closeTime.format('HH:mm')} | Motivo: ${isBeforeOpen ? 'Antes de abrir' : 'Depois de fechar'}`);
        return false;
    }
...
export async function findAvailableSlots({
    ...
    for (let i = 0; i < daysToCheck; i++) {
        ...
        const { openHour, openMin, closeHour, closeMin, hasRule } = getOpenHours(dateStr, scheduleRules);
        console.log(`[findAvailableSlots] 🕒 Data: ${dateStr}, Início detectado: ${openHour}:${openMin}${hasRule ? ' (via regra)' : ' (fallback 7h)'}, Fim: ${closeHour}:${closeMin}`);
        ...
        while (timeCursor.isBefore(closeTime)) {
            ...
            if (!conflict) {
                availableSlots.push({
                    start: slotStart.format('YYYY-MM-DD HH:mm'), // Local string for AI/Tool communication
                    time: slotStart.format('HH:mm'),            // Local time for Client
                    day: slotStart.format('DD/MM'),             // Local day
                    label: slotStart.format('DD/MM (ddd) HH:mm') // Descriptive label
                });
                slotsFound++;
            }
            timeCursor.add(5, 'minutes');
...
export async function bookAppointment({
    ...
    // Parse the start time: Use the simplified local format (BRT) from the AI
    // If the string contains 'T' or 'Z', it might be an ISO string, so we handle both.
    const start = startsAt.includes('T') || startsAt.includes('Z') 
        ? moment(startsAt).tz(TIMEZONE) 
        : moment.tz(startsAt, "YYYY-MM-DD HH:mm", TIMEZONE);
    ...
    if (!isTimeValid(start, end, scheduleRules)) {
        ...
        return { error: true, message: "Horário indisponível (fora do expediente ou bloqueado)." };
    }
    ...
    if (existing) {
        for (const apt of existing) {
            ...
            if (checkOverlap(start, end, aptStart, aptEnd)) {
                console.warn(`[bookAppointment] ❌ Conflito Agendamento: ${start.format('HH:mm')} com ${apt.customer_name}`);
    ...
    if (blocks) {
        for (const block of blocks) {
            const bStart = moment.tz(block.starts_at, TIMEZONE);
            const bEnd = moment.tz(block.ends_at, TIMEZONE);
            if (checkOverlap(start, end, bStart, bEnd)) {
                console.warn(`[bookAppointment] ❌ Conflito Bloqueio: ${start.format('HH:mm')} com ${block.reason || 'bloqueio'}`);
                return { error: true, message: "Este horário possui um bloqueio manual no sistema." };
            }
        }
    }
