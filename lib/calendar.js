...
export function calculateTotalDuration(services) {
    if (!Array.isArray(services)) services = [services];
    return services.reduce((total, s) => {
        // Try exact match
        if (SERVICE_DURATIONS[s]) return total + SERVICE_DURATIONS[s];

        // Try partial match (e.g. "Manutenção" in "Manutenção de Unha")
...
export async function bookAppointment({
    ...
    const finalDuration = duration || calculateTotalDuration(serviceList);
