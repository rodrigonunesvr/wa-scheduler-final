...
export async function calculateTotalDuration(services) {
    ...
    const { data: dbServices } = await supabase.from('services').select('*').eq('active', true).eq('is_hidden', false);
    const serviceMap = {};
    if (dbServices) {
        dbServices.forEach(s => serviceMap[s.name] = s.duration);
    }

    return services.reduce((total, s) => {
        if (serviceMap[s]) return total + serviceMap[s];
...
export async function calculateTotalPrice(services) {
    ...
    const serviceMap = {};
    if (dbServices) {
        dbServices.forEach(s => serviceMap[s.name] = s.price);
    }
    ...
    return services.reduce((total, s) => {
        ...
        for (const [name, price] of Object.entries(serviceMap)) {
            if (s.toLowerCase().includes(name.toLowerCase())) return total + price;
        }

        return total + 0; // Default fallback
    }, 0);
}
...
export async function bookAppointment({
    ...
    // VALIDATION: Ensure all services are active and not hidden
    const { data: activeDB } = await supabase.from('services').select('name').eq('active', true).eq('is_hidden', false);
    const activeNames = (activeDB || []).map(s => s.name.toLowerCase());
    const displayNames = (activeDB || []).map(s => s.name).join(', ');

    for (const s of serviceList) {
        const found = activeNames.some(name => s.toLowerCase().includes(name));
        if (!found) {
            return { error: true, message: `O serviço "${s}" não está disponível no momento. Nossas opções atuais são: ${displayNames}.` };
        }
    }
...
export async function updateAppointment({
    ...
    for (const s of serviceList) {
        const found = activeNames.some(name => s.toLowerCase().includes(name));
        if (!found) {
            return { error: true, message: `O serviço "${s}" não está disponível no momento. Disponíveis: ${displayNames}.` };
        }
    }
